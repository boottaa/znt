...
*   `max_bytes`: Максимальный размер входного контекста в байтах (по умолчанию: `25000`).
*   `max_embed_bytes`: Максимальный размер пакета эмбеддингов в байтах (по умолчанию: `32768`).

#### `[semantic]` — Настройки семантического анализа
*   `mode`: Режим работы семантического анализа (`llm` или `fast`). `llm` использует нейросетевые модели, а `fast` работает автономно без LLM (0ms оверхеда, детерминированный разбор AST, TF-IDF теги и определение ролей).
*   `concurrency.semantic_workers`: Число параллельных воркеров извлечения семантики (по умолчанию: `3`).
*   `throttling.delay_ms`: Базовая задержка между запросами к LLM в миллисекундах (по умолчанию: `10`).
